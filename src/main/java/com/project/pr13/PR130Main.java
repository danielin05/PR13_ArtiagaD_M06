...
public class PR130Main {
    ...
    public static void imprimirDadesPersones(NodeList persones){
        ...
        for (int cnt = 0; cnt < persones.getLength(); cnt++){
            ...
            NodeList nomsPersonas = elm.getElementsByTagName("nom");

            NodeList cognomsPersonas = elm.getElementsByTagName("cognom");

            NodeList edatPersonas = elm.getElementsByTagName("edat");

            NodeList ciutatPersonas = elm.getElementsByTagName("ciutat");

            String nom = nomsPersonas.item(0).getTextContent();

            String cognom = cognomsPersonas.item(0).getTextContent();

            String edat = edatPersonas.item(0).getTextContent();

            String ciutat = ciutatPersonas.item(0).getTextContent();

            System.out.println(String.format("%-8s %-14s %-5s %-9s", nom, cognom, edat, ciutat));
